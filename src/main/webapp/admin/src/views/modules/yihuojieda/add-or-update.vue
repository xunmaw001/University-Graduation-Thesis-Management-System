...
export default {
  ...
  created() {
	this.ruleForm.huifuriqi = this.getCurDate()
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yihuojieda/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yihuojiedaCrossAddOrUpdateFlag = false;
						 this.parent.search();
